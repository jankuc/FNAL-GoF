function [H, pVal, KSstatistic] = test_wRKS2(x1, w1, x2, w2, varargin)
%KSTEST2 Two-sample Renyi modified Kolmogorov-Smirnov goodness-of-fit hypothesis test.
%   H = test_wKS2(X1,w1,X2,w2) performs a Kolmogorov-Smirnov (K-S) test
...
logical = sampleCDF1 > qa & sampleCDF1 <qb & sampleCDF2 >qa & sampleCDF2 < qb;

sampleCDF1 = sampleCDF1(logical);
sampleCDF2 = sampleCDF2(logical);

% Compute the test statistic of interest.

switch tail
  case  0      %  2-sided test: T = max|F1(x) - F2(x)|.
    deltaCDF1  =  abs(sampleCDF1 - sampleCDF2)./sampleCDF1;
    deltaCDF2  =  abs(sampleCDF1 - sampleCDF2)./sampleCDF2;
...
deltaCDF1 = deltaCDF1(deltaCDF1 < 1 );
deltaCDF2 = deltaCDF2(deltaCDF2 < 1 );
allDists = [deltaCDF1(:); deltaCDF2(:)];
allDists = allDists(isfinite(allDists));
KSstatistic   =  max(allDists);
...
if tail ~= 0        % 1-sided test.
  ...
  pVal = lambda2pVal(lambda*sqrt(alpha/(1-alpha)));
...
j       =  (0:100)';
jj = (-1).^(j)./(2*j+1).*exp(-((2*j+1).^2*pi^2)/(8*lambda^2));
pVal  =  4/pi * sum(jj); % = K
